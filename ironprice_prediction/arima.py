...
class Arimamultivarient(APIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        last_date = str(MultivarientData.objects.latest('date').date - datetime.timedelta(days=250))
        ...
        date_valid = MultivarientData.objects.exclude(date__gt=end_date).exclude(date__lt=start_date)
        if not date_valid:
        ...
        test['oilpriceprediction'] = arima.predict(test.index.min(), test.index.max())
        ...
        ironpredicted_data = test[['date', 'ironpriceprediction']].values.tolist()
        oilactual_data = test[['date', 'oil_price']].values.tolist()
        oilpredicted_data = test[['date', 'oilpriceprediction']].values.tolist()
        ironmetrics = forecast_accuracy(test['iron_price'], test['ironpriceprediction'])
        oilmetrics = forecast_accuracy(test['oil_price'], test['oilpriceprediction'])
        return Response({'actual_irondata': ironactual_data, 'predicted_irondata': ironpredicted_data,'actual_oildata': oilactual_data, 'predicted_oildata': oilpredicted_data,'ironmape': ironmetrics.get('mape', 0)*100, 'oilmape': oilmetrics.get('mape', 0)*100})
class ArimaMultivarientForecast(APIView):

    def get(self, request, *args, **kwargs):
        n_steps = int(self.request.query_params.get('nsteps', 10))

        last_date = MultivarientData.objects.latest('date').date + datetime.timedelta(days=30)
        ...
        arima = SARIMAX(endog=oildf, order=(3, 0, 4), freq='M', seasonal_order=(0, 1, 1, 6), trend='t',
                        enforce_stationarity=False, enforce_invertibility=False).fit()
        df['oilpriceprediction']=arima.predict(date_index.min(), date_index.max())
        ...
        df['date'] = df.index
        predicted_data = df[['date', 'ironpriceprediction','oilpriceprediction']].values.tolist()
